...
capture log close
log using cleaner.log, replace

local EY = int(10000*uniform()) // NO seed; needs to be random
! gunzip -fc ${sourcedata}/census80900010.dat.gz > tmp`EY'.dat
...
quietly infix                 ///
  ...
  byte    qeduc      98-98    ///
  byte    qschool    99-99    ///
  byte    qclasswk   100-100  ///
  byte    quhrswor   101-101  ///
  byte    qwkswork   102-102  ///
  byte    qincwage   103-103  ///
  using tmp`EY'.dat
! rm tmp`EY'.dat
...
label define occ1990_lbl 387 `"Teachers aides"', add
...
label define occ1990_lbl 443 `"Waiters assistant"', add
...
* create variables to match with other datasets
* Completed schooling
generat hgc = .
replace hgc = 0    if inlist(educd,0,1,2,11,12)
replace hgc = 1    if inlist(educd,14)
replace hgc = 2    if inlist(educd,10,13,15)
replace hgc = 3    if inlist(educd,16)
replace hgc = 4    if inlist(educd,17)
replace hgc = 5    if inlist(educd,21,22)
replace hgc = 6    if inlist(educd,20,23)
replace hgc = 7    if inlist(educd,24,25)
replace hgc = 8    if inlist(educd,26)
replace hgc = 9    if inlist(educd,30)
replace hgc = 10   if inlist(educd,40)
replace hgc = 11   if inlist(educd,50)
replace hgc = 11.5 if inlist(educd,61)
replace hgc = 12   if inlist(educd,60,62,63,64)
replace hgc = 12.5 if inlist(educd,65)
replace hgc = 13   if inlist(educd,70,71)
replace hgc = 14   if inlist(educd,80,81,82,83)
replace hgc = 15   if inlist(educd,90)
replace hgc = 15.5 if inlist(educd,100)
replace hgc = 16   if inlist(educd,101)
replace hgc = 17   if inlist(educd,110)
replace hgc = 18   if inlist(educd,111,114)
replace hgc = 19   if inlist(educd,112,115)
replace hgc = 20   if inlist(educd,113,116)

gen ged     = educd==64
gen gradHS  = hgc>=12
* gen grad4yr = hgc>=16
gen grad4yr = hgc>=15.5
...
replace nativeamer = 1 if race==3
replace asian      = 1 if inrange(race,4,6)
replace otherrace  = 1 if inrange(race,7,.)

foreach var of varlist white black nativeamer asian otherrace {
    replace `var' = 0 if hispanic == 1
}
drop race

generat race = 1 if white==1
replace race = 2 if black==1
replace race = 3 if hispanic==1
replace race = 4 if nativeamer==1
replace race = 5 if asian==1
replace race = 6 if otherrace==1
assert race != .

lab def vlrace 1 "White" 2 "Black" 3 "Hispanic" 4 "Native Amer" 5 "Asian" 6 "Other"
lab val race vlrace

* Birth year
gen birthyr = year-age

* Enrolled in school at time of survey
generat enrolled = 0
...
* Employment status
generat emp = 0
replace emp = 1 if empstat==1 & uhrswork>10

* New FT definition: 35+ hours AND 40+ weeks
generat empFT = 0
replace empFT = 1 if inrange(uhrswork ,35,.) & (inrange(wkswork1,40,52) | inrange(wkswork2,4,6))

generat empPT = 0
replace empPT = 1 if inrange(uhrswork,10,35) & empFT==0
* generat empFT = 0
* replace empFT = 1 if uhrswork>=35
* generat empPT = 0
* replace empPT = 1 if inrange(uhrswork,10,34.99999999)


* Wage and salary income
* Missing values coded as 999999; change these to .
recode incwage (999999 = .)

* Deflate earnings to 1982-84 dollars
replace incwage = incwage/0.824 if year==1980
replace incwage = incwage/1.307 if year==1990
replace incwage = incwage/1.722 if year==2000
replace incwage = incwage/1.771 if year==2001
replace incwage = incwage/1.799 if year==2002
replace incwage = incwage/1.840 if year==2003
replace incwage = incwage/1.889 if year==2004
replace incwage = incwage/1.953 if year==2005
replace incwage = incwage/2.016 if year==2006
replace incwage = incwage/2.073 if year==2007
replace incwage = incwage/2.153 if year==2008
replace incwage = incwage/2.145 if year==2009
replace incwage = incwage/2.181 if year==2010
replace incwage = incwage/2.249 if year==2011
replace incwage = incwage/2.296 if year==2012
replace incwage = incwage/2.330 if year==2013
replace incwage = incwage/2.367 if year==2014
replace incwage = incwage/2.370 if year==2015
replace incwage = incwage/2.400 if year==2016

generat hrlywage = incwage/(uhrswork*wkswork1) if year<=2007
replace hrlywage = incwage/(uhrswork*7)        if year>=2008 & wkswork2==1 //  1-13 weeks
replace hrlywage = incwage/(uhrswork*20)       if year>=2008 & wkswork2==2 // 14-26 weeks
replace hrlywage = incwage/(uhrswork*33)       if year>=2008 & wkswork2==3 // 27-39 weeks
replace hrlywage = incwage/(uhrswork*44)       if year>=2008 & wkswork2==4 // 40-47 weeks
replace hrlywage = incwage/(uhrswork*48)       if year>=2008 & wkswork2==5 // 48-49 weeks
replace hrlywage = incwage/(uhrswork*51)       if year>=2008 & wkswork2==6 // 50-52 weeks

genera lnearnings = ln(incwage)
rename incwage earnings

genera lnwage = ln(hrlywage)
rename hrlywage wage

keep serial pernum year age birthyr perwt black hispanic white asian nativeamer otherrace race female lnearnings earnings lnwage wage uhrswork empPT empFT wkswork1 wkswork2 hgc ged gradHS grad4yr enrolled
compress

save ${finaldata}/Census_1980_1990_2000_2010, replace
!gzip ${finaldata}/Census_1980_1990_2000_2010.dta
...
forvalues X = 1/`NUMPIECES' {
    preserve
        ...
        save     ${finaldata}/Census_1980_1990_2000_2010_pt`X', replace
        !gzip -f ${finaldata}/Census_1980_1990_2000_2010_pt`X'.dta
    restore
}
