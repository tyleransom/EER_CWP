...
set more off
capture log close
log using compare_premia_earnings.log, replace
...
append using CPS/cps_regs_earnings.dta, gen(CPS2)
replace dataset="CPS" if CPS2==1
drop `DATALIST'
order dataset, last
    ...
    foreach FEM in 0 1 {
		...
		foreach Y in Raw Mincer HLT {
			foreach A in 18 25 26 30 35 45 {
                ...
                if `A'==35 local A2 44
                if `A'==45 local A2 54
				twoway ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y'"      & dataset=="CPS"                  , lwidth(medthick) lcolor(blue)) ///
					   ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y'"      & dataset=="ACS"                  , lwidth(medthick) lcolor(red)) ///
				       ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y' earn" & dataset=="CPS"                  , lwidth(medthick) lcolor(ltblue)) ///
					   ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y' earn" & dataset=="ACS"                  , lwidth(medthick) lcolor(erose)) ///
				, legend(order(1 2 3 4) label(1 "CPS Wage") label(2 "ACS Wage") label(3 "CPS Earn") label(4 "ACS Earn") cols(2) symxsize(10) keygap(1) ) ///
				...
				*xscale(range(`minyear' `maxyear')) xlabel(`minyear'(4)`maxyear') yscale(range(-.2 .8)) ylabel(-.2(.2).6) xtitle(Birth Year) ytitle(Wage Premium) graphregion(color(white))
				graph export PDFs/line_wageVearnings_`PRE'male_FT_`A'`A2'_`X'_`Y'.pdf, replace
				graph export  PSs/line_wageVearnings_`PRE'male_FT_`A'`A2'_`X'_`Y'.eps, replace
				restore
			}
		}
    }
    ...
    foreach FEM in 0 1 {
		...
		foreach Y in Raw {
			foreach A in 18 25 26 30 35 45 {
                ...
                if `A'==35 local A2 44
                if `A'==45 local A2 54
				twoway ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y'"      & dataset=="CPS"                  , lwidth(medthick) lcolor(navy)) ///
				       ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y'NoImp" & dataset=="CPS"                  , lwidth(medthick) lcolor(ltblue)) ///
				, legend(order(1 2) label(1 "CPS Wage") label(2 "CPS No Imputes") cols(2) symxsize(10) keygap(1) ) ///
				xscale(range(`minyear' `maxyear')) xlabel(`minyear'(4)`maxyear') yscale(range(`=`ymin'-.05' `=`ymax'+.05')) ylabel(`ymin'(.2)`ymax') xtitle(Birth Year) ytitle(Wage Premium) graphregion(color(white))
				*xscale(range(`minyear' `maxyear')) xlabel(`minyear'(4)`maxyear') yscale(range(-.2 .8)) ylabel(-.2(.2).6) xtitle(Birth Year) ytitle(Wage Premium) graphregion(color(white))
				graph export PDFs/line_CPSimputes_`PRE'male_FT_`A'`A2'_`X'_`Y'.pdf, replace
				graph export  PSs/line_CPSimputes_`PRE'male_FT_`A'`A2'_`X'_`Y'.eps, replace
				restore
			}
		}
    }
		...
		foreach Y in Raw HLT Mincer {
			foreach A in 18 25 26 30 35 45 {
				...
				twoway ( rarea `X'hi `X'lo birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y'"      & dataset=="NLSY" & birthyr<=1964 , color(gs14)) ///
					   ...
					   ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y' earn" & dataset=="CPS"                  , lwidth(medthick) lcolor(black) lpattern(dash)) ///
					   ( line  `X'         birthyr if female==`FEM' & empFT & age1==`A' & N>=`obscut' & controls=="`Y' earn" & dataset=="ACS"                  , lwidth(medthick) lcolor(black) lpattern(dot)) ///
				...
				xscale(range(`minyear' `maxyear')) xlabel(`minyear'(4)`maxyear') yscale(range(`=`ymin'-.05' `=`ymax'+.05')) ylabel(`ymin'(.2)`ymax') xtitle(Birth Year) ytitle(Wage Premium) graphregion(color(white))
				*xscale(range(`minyear' `maxyear')) xlabel(`minyear'(4)`maxyear') yscale(range(-.2 .8)) ylabel(-.2(.2).6) xtitle(Birth Year) ytitle(Wage Premium) graphregion(color(white))
				graph export PDFs/line_earnings_`PRE'male_FT_`A'`A2'_`X'_`=subinstr("`Y'"," ","",.)'.pdf, replace
				graph export  PSs/line_earnings_`PRE'male_FT_`A'`A2'_`X'_`=subinstr("`Y'"," ","",.)'.eps, replace
				restore
			}
		}
    }
